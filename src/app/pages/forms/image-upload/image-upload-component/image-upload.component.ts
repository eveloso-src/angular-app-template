...
export class ImageUploadComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private imageUploadService: ImageUploadService,
    private addCapabilityService: AddCapabilityService, private imageUploadValidator: ImageUploadValidator,
    ...
    addCapabilityService.getPostedCapability().subscribe(value => {
      this.capabilitiesDropdownList = this.capabilitiesDropdownList.concat(value.data)
    });
    imageUploadService.getUploadedImagePercentage().subscribe(value => {
      if (value.uploadedPercentage !== 'NaN%') {
        this.imageuploadPercentage = value.uploadedPercentage;
      }
      });
    }
  ...
  ngOnInit() {
    this.imageUploadForm = this.formBuilder.group({
      ...
      imageName: ['', Validators.required, this.imageUploadValidator.checkImageName.bind(this.imageUploadValidator)],
      tagName: ['', Validators.required, this.imageUploadValidator.checkTagName.bind(this.imageUploadValidator)],
      author: [''],
      capabilities: ['', Validators.required],
      devices: [''],
      uploadFile: ['', [Validators.required, this.imageUploadValidator.fileUploadValidator]],
    });
    ...
    this.imageUploadService.getAllCapabilities()
      .subscribe((data: CapabilityModel[]) => {
        if (typeof data !== undefined && data.length > 1) {
          ...
          this.capabilitiesDropdownList = capList;
        }
      },
        err => {
          throw err;
...
//    console.log(item);
  ...
  openFileBrowser(event: any) {
    ...
    const element: HTMLElement = document.getElementById('inputGroupFile01') as HTMLElement;
    element.click();
  ...
  onImageUpload() {
    ...
    const imageUploadModel = new ImageUploadModel(this.imageUploadForm.value['name'], this.imageUploadForm.value['imageName'], this.imageUploadForm.value['tagName'], this.imageUploadForm.value['author'], capabilitiesSelectedItems, this.imageUploadForm.value['devices']);
    console.log('Submitted the form');
    this.ngxService.start();
    this.imageUploadService.onImageUpload(imageUploadModel).then(() => {
      this.imageUploadService.tagImageToRegistry(imageUploadModel.image_name, imageUploadModel.image_tag).then(() => {
        this.imageUploadService.pushImageToRegistry(imageUploadModel.image_name, imageUploadModel.image_tag).then(() => {
          this.imageUploadService.postImageMetadata(imageUploadModel);
          this.toastr.success('Image ' + imageUploadModel.image_name + ' pushed to Registry successfully', 'Success', {
            disableTimeOut: true,
            closeButton: true,
          });
        })
