...
export class EnvironmentalCapabilityComponent implements OnInit {
  ...
  onSubmit() {
    ...
    this.addCapabilityService.addCapability(capabilityModel).then((data) => {
      this.toastr.success('Environment Capability ' + capabilityModel.name + ' Added Successfully', 'Success', {
        disableTimeOut: true,
        closeButton: true,
      });
    }).catch((err) => {
